...
from helpers.eip1167 import extract_eip1167_address, is_eip1167_contract
from helpers.fork import AccountDBFork
from helpers.gas_meters import GasMeter, NoGasMeter, ProfilingGasMeter
...
def _unpack_asan_ptr(asan_ptr, size):
    ...
    if ptr + size > hi:
        raise MemoryAccessViolation(
            f"tried to access {ptr + size} but bounds are | {lo} | ... | {hi} |"
        )
...
class Env:
    _singleton = None
    _random = random.Random("vyper-fuzz")  # something reproducible
    ...
    def _init_vm(self, reset_traces=True):
        ...
        # Sanitizer computation
        class SanitizingComputation(self.vm.state.computation_class):
